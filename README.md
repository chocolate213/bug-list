...
2. 使用Maven默认构建的web.xml文件将会导致jsp页面中无法读取requestScope中的内容。解决办法：将web.xml的声明替换为如下代码(这种方式可同时处理idea中Servlet等元素报错的问题)：
...
4. Maven编译war文件时会忽略类路径下的*.xml(mapper.xml)文件导致出现如下异常（原因：没有找到映射文件，编译时在target文件夹中没有mapper.xml文件）：
...
6. Spring容器只支持一个<context:property-placeholder/>配置，而且只会读取到第一个配置文件，其他的会被忽略。
...
9. 解决Get请求中文乱码问题：在Tomcat配置文件的Connector节点配置如下参数
...
12. Servlet Api与JSP支持包由Servlet容器（Tomcat）提供，所以在pom.xml文件中不用再次提供该文件，所以在配置文件中将其编译范围指定为provide（已提供）
