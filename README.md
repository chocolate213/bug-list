...
> HashTable: 按照key降序排序；
> HashMap: 按照key的某种顺序排序（无论put元素的顺序如何，都会按照某种顺序排序），但是既不是升序，也不是降序，也不是原序；
> LinkedHashMap: 按照key原序排序；
> TreeMap：按照key升序排序；
> ConcurrentHashMap：按照key的某种顺序排序（无论put元素的顺序如何，都会按照某种顺序排序），但是与HashMap的排序方式不同。

* JDK1.8中，keySet方法以及entrySet方法将会按照如下顺序排序：
> HashTable: 按照key降序排序；
> HashMap: 按照key升序排序（**与JDK1.7不同**）；
> LinkedHashMap: 按照key原序排序；
> TreeMap：按照key升序排序；
> ConcurrentHashMap：按照key升序排序（**与JDK1.7不同**）。
